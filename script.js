document.addEventListener('DOMContentLoaded', () => {
                ...
                if (typeof fetchMetadata === 'function') {
                    try {
                        ...
                        if (metadata) {
                            ...
                            // Handle Deferred Summary (for Imdb/Games)
                            if (metadata.deferredSummary) {
                                // Update first with image
                                updateItemData(activeCategory, dateAdded, updates);

                                // Then fetch summary
                                const extra = await metadata.deferredSummary();
                                if (extra) {
                                     updates.summary = extra.summary || updates.summary;
                                     // Also update image if it got better
                                     updates.imageUrl = extra.imageUrl || updates.imageUrl;
                                     updateItemData(activeCategory, dateAdded, updates);
                                }
                                return; // Stop here as we handled updates
                            }
                        }
                    } catch (e) {
    ...
    // Shared IMDb Fetcher with Deferred Summary (Ported from Base)
    async function fetchImdbMetadata(title, year) {
        try {
            // Append year to query if available for better accuracy
    ...
    async function fetchMovieMetadata(title, year) {
        return fetchImdbMetadata(title, year);
    }

    async function fetchGameMetadata(title, year) {
        // Use IMDb for Games too (Better Images)
        return fetchImdbMetadata(title, year);
    }
